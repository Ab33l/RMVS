...
session_start();
$rmId = $_SESSION['rmId'];        // Collecting data from query string
if(!is_numeric($rmId)){ // Checking data it is a number or not
echo "Data Error" . $_SESSION['rmId'];    
exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $stmt = $db->prepare("UPDATE staff SET status=?, modified=? WHERE staffId = $rmId");
    $stmt->bind_param('ss', $callbackoptions, $nowdate);
    $_SESSION["staffupdate_success"] = true;

    $stmt->execute();
    $stmt->close();
    header("location: /gohub/gohubAdmin/darkpanBO/staffmgmt.php");
}

$result = uniqid();

$query = $db->query("SELECT * FROM staff WHERE staffId = $rmId");
if($query->num_rows > 0){ 
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="staffmgmt.php" class="nav-item nav-link active"><i class="fa fa-table me-2"></i>Staff Mgmt</a>
                    <?php    } ?>
                </div>
            </nav>
        </div>
        ...
        <div class="content">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" autocomplete="off">
                        <div class="bg-secondary rounded h-100 p-4" style="width:70%;margin-left:15%;">
                            <h6 class="mb-4">RM Management Form</h6>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput"
                                    placeholder="RM's AB Number" name="remsname" value="<?php echo $row['abNumber']; ?>" disabled>
                                <label for="floatingInput">RM's AB Number</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput"
                                    placeholder="RM's Phone Number" name="remsac" value="<?php echo $row['phoneNumber'];?>" disabled>
                                <label for="floatingInput">RM's Phone Number</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="email" class="form-control" id="floatingInput"
                                    placeholder="RM's Email Address" name="bensac" value="<?php echo $row['email_address']; ?>" disabled>
                                <label for="floatingInput">RM's Email Address</label>
                            </div>
                            <hr>
                            <label for="floatingSelect">Update RM Status:  </label>
                            ...
                            <button type="submit" class="btn btn-primary">Update RM Status</button>
                            <button class="btn btn-warning" style="margin-left:60%;" onclick="location.href='staffmgmt.php'">Cancel</button>
                            </div>
                        </div>
                        <?php }else{ ?>
                        <p>Specific RM's Details Being Processed</p>
                    <?php } ?>
                    </form>
            </div>
            <!-- Widget End -->
