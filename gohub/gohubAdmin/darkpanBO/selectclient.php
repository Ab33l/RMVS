...
session_start();
//$sessionId = $_SESSION['sessionId'];        // Collecting data from query string
$sessionId = $_GET['sessionId'];
if(!is_numeric($sessionId)){ // Checking data it is a number or not
echo "Data Error" . $_SESSION['sessionId'];    
exit;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $stmt = $db->prepare("UPDATE customers SET status=?, modified=? WHERE sessionId = $sessionId");
    $stmt->bind_param('ss', $callbackoptions, $nowdate);
    $_SESSION["customerupdate_success"] = true;

    $stmt->execute();
    $stmt->close();
    header("location: /gohub/gohubAdmin/darkpanBO/clientmgmt.php");
}

$result = uniqid();

$query = $db->query("SELECT * FROM customers WHERE sessionId = $sessionId");
if($query->num_rows > 0){ 
            ...
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <?php
                            ...
                            ?>
                    <a href="clientmgmt.php" class="nav-item nav-link active"><i class="fa fa-keyboard me-2"></i>Client Mgmt</a>
        ...
        <div class="content">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" autocomplete="off">
                        <div class="bg-secondary rounded h-100 p-4" style="width:70%;margin-left:15%;">
                            <h6 class="mb-4">Client Management Form</h6>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput"
                                    placeholder="Client's Full Name" name="remsname" value="<?php echo $row['full_name']; ?>" disabled>
                                <label for="floatingInput">Client's Full Name</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput"
                                    placeholder="Client's Phone Number" name="remsac" value="<?php echo $row['phoneNumber'];?>" disabled>
                                <label for="floatingInput">Client's Phone Number</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="email" class="form-control" id="floatingInput"
                                    placeholder="Client's Email Address" name="bensac" value="<?php echo $row['email_address']; ?>" disabled>
                                <label for="floatingInput">Client's Email Address</label>
                            </div>
                            <hr>
                            <label for="floatingSelect">Update Client Status:  </label>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox1" name="callbackoptions" value="1" default>
                                <label class="form-check-label" for="inlineCheckbox1">Activate</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox2" name="callbackoptions" value="2">
                            ...
                            <button class="btn btn-warning" style="margin-left:60%;" onclick="location.href='clientmgmt.php'">Cancel</button>
                            </div>
                        </div>
                        <?php }else{ ?>
                        <p>Specific Client Details Being Processed</p>
                    <?php } ?>
                    </form>
            </div>
            <!-- Widget End -->
